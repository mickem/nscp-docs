...
They are mearly a glorified way to save yourself some typing for instance `nscp sys` will yield the same result as saying `nscp client --module CheckSystem`.

| Command | Description |
|---------|-------------|
eventlog | Inject event log message into the eventlog (mainly for testing eventlog filtering and setup)|
...
After whi h we can list all commands (queries):
...
Commands in this context is actual check commands which generaly checks some aspect of you system.
