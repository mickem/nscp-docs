...
<dl>
  <dt>To check where the configuratio is stored you can trun the following command :  : </dt>
  <dd>
...
<dl>
  <dt>To add multipl service you need to first create the services :  : </dt>
  <dd>nscp service --install --name nscp2</dd>
</dl>
<dl>
  <dt>And then edit the start command so you can override the configuration. The key to look for in the registry is HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\<NAME OF SERVICE> and there you can modify the ImagePath :  : </dt>
  <dd>"C:\Program Files\NSClient++\nscp.exe" service --run --name nscp --settings ini://${shared-path}/nsclient-2.ini</dd>
</dl>
